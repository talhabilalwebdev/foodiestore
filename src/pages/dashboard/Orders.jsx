...
const OrdersAdmin = () => {
  ...
  const fetchOrders = async () => {
    try {
      setLoading(true);
      const res = await axios.get("https://foodiebackend-1-ef18.onrender.com/api/orders");
  ...
  const handleStatusChange = async (id, newStatus) => {
    try {
      await axios.put(`https://foodiebackend-1-ef18.onrender.com/api/orders/update/${id}`, {
        status: newStatus,
      });
      setOrders((prev) =>
        prev.map((order) =>
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`https://foodiebackend-1-ef18.onrender.com/api/orders/delete/${id}`);
